...
Normal path:
`student event -> adaptive_tutor -> render response`

Budget:
- one blocking pedagogical Skill call;
- at most one blocking external rendering Skill call, and only when necessary;
- zero dependence on `state_observer`.

## Background path

After rendering:
- enqueue `state_observer`;
- optionally build `visual_explainer`;
...
While `lecture_builder` is being viewed, the host should *optionally* run:
