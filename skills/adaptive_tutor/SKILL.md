...
`lesson_opener -> lecture_builder` phase. Choose the smallest interaction that produces useful
...
Run while `lecture_builder` is being viewed. Do not require a student reply or update mastery.
Prepare one high-information post-lecture probe, one to three misconception patterns tied to the
taught content, a cheap Chinese fallback for each branch, and optional `visual_explainer` briefs
...
Request `visual_explainer` only when manipulation, comparison, geometry, algorithm tracing, or a
...
`lecture_builder` only for a substantial missing sub-concept that needs structured re-teaching.
